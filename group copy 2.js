...
function expandDataByOneLabel(nodes, links, curNode) {
  ...
  curNode.node_list.forEach(node => {
    node.as_source_links.forEach(link => {
      ...
      if(nodes.findIndex(gn => gn.id === link.group_target_id) === -1) {
        if(nodes.findIndex(n => n.id === link.origin_target_id) !== -1) {
          brandNewLink.target = link.origin_target_id
    ...
    node.as_target_links.forEach(link => {
      ...
      if(nodes.findIndex(gn => gn.id === link.group_source_id) === -1) {
        if(nodes.findIndex(n => n.id === link.origin_source_id) !== -1) {
          brandNewLink.source = link.origin_source_id
          newLinks.push(brandNewLink)
        }
  ...
  // console.log('new', JSON.stringify(newNodes), JSON.stringify(newLinks))
  // debugger
  update(newNodes, newLinks)
}
    ...
    nodes.forEach(n => {
      if(n.label === group) {
        n.as_source_links = links.filter(link => link.source === n.id)
          // .map(link => ({
          //   id: link.id,
          //   // group_target_id: link.group_target_id,
          //   // origin_target_id: link.origin_target_id,
          //   // group_source_id: link.group_source_id,
          //   // origin_source_id: link.origin_source_id,
          // }))
